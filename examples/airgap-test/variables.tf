...
variable "workload_node_count" {
  description = "Number of workload nodes (spot instances)"
  type        = number
  default     = 3

  validation {
    condition     = var.workload_node_count >= 1 && var.workload_node_count <= 10
    error_message = "workload_node_count must be between 1 and 10."
  }
}
